...
int ParseMultipart(char *body, int bodyLen, const char *boundary,
                   char **filename, char **fileData, int *fileSize) {
...
void *ClientThread(void *arg) {
  ...
  if (requestBody != NULL && strstr(requestBody, "\r\n\r\n") != NULL) {
    ...
    if (strcmp(method, "POST") == 0 && contentLength > 0) {
      ...
      if (remainingBody > 0) {
        ...
        while (totalRead < remainingBody) {
          int r = recv(c, requestBody + bodyLen + totalRead,
                       remainingBody - totalRead, 0);
          if (r > 0)
            totalRead += r;
          else
            break;
    ...
    if (strcmp(method, "POST") == 0 && strlen(boundary) > 0) {
      ...
      if (ParseMultipart(requestBody, bodyLen, boundary, &filename, &fileData,
                         &fileSize)) {
        // Create full path
        char fullPath[2048];
        ...
        if (f != NULL) {
          ...
          char *error =
              ...
              "Error\r\n\r\n<html><body><h2>Failed to save file</h2><p>Check "
              "path and permissions</p></body></html>";
