    ...
    while (1)
    {
        ...
        if (n > 0)
        {
            ...
            if (strncmp(buffer, "SEND ", 5) == 0)
            {
                ...
                int sender_port;
                if (sscanf(buffer + 5, "%s %s %s %d", filename, target_name, sender_name, &sender_port) == 4)
                {
                    if (strcmp(target_name, my_name) == 0)
                    {
                        ...
                        if (client_sock >= 0)
                        {
                            ...
                            recv(client_sock, &file_size_network, sizeof(file_size_network), 0);
...
void send_file_to_client(const char *filename, const char *target_name)
{
    ...
    sprintf(send_message, "SEND %s %s %s %d", filename, target_name, my_name, my_listen_port);
